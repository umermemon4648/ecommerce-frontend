...
const Login = () => {
  ...
  const {loading, isAuthenticate, error} = useSelector((state)=> state.user)
    ...
    const loginSubmited = (e)=>{
      ...
      // dispatch(getLoginUser(email, password)).then(()=> history(`/`))
      dispatch(emptyCart())
      dispatch(getLoginUser(email, password)).then(()=> history(`/`))
    }
// let redirect = location.search?location.search.split("=")[1]:"/"
  ...
  <div className="w-full max-w-md space-y-8">
    ...
    <form onSubmit={loginSubmited} className="mt-8 space-y-7" method="POST">
     ...
     <Link to={"/auth/newUser"}> <a href="#" className="text-indigo-600 font-medium inline-flex space-x-1 items-center"><span>Register now </span><span><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
